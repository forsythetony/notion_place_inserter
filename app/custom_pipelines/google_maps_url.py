"""Custom pipeline for Google Maps URL: ExtractGoogleMapsUri -> format."""
...
class FormatGoogleMapsStep(PipelineStep):
    """Format as Notion url or rich_text."""
...
class GoogleMapsUrlPipeline(Pipeline):
    """Resolve Google Maps link from Google Places googleMapsUri or place_id fallback."""
    ...
    def pipeline_id(self) -> str:
        return f"google_maps_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [
            ExtractGoogleMapsUri(),
            FormatGoogleMapsStep(self._prop_name, self._prop_schema),
        ]
