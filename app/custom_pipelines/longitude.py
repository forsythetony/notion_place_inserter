"""Custom pipeline for Longitude: extract from place location (no LLM)."""
...
from app.pipeline_lib.steps.google_places import ExtractLongitude
from app.pipeline_lib.steps.notion_format import format_value_for_notion


class FormatLongitudeStep(PipelineStep):
    """Format as Notion number."""
...
class LongitudePipeline(Pipeline):
    """Resolve longitude from place location. Returns None if not in API response."""
    ...
    def pipeline_id(self) -> str:
        return f"longitude_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [
            ExtractLongitude(),
            FormatLongitudeStep(self._prop_name, self._prop_schema),
        ]
