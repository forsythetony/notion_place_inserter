"""Custom pipeline for Address: ExtractFormattedAddress -> format."""
...
from app.pipeline_lib.steps.google_places import ExtractFormattedAddress
from app.pipeline_lib.steps.notion_format import format_value_for_notion


class FormatAddressStep(PipelineStep):
    """Format as Notion rich_text or url depending on schema."""
...
class AddressPipeline(Pipeline):
    """Resolve address from Google Places formattedAddress."""
    ...
    def pipeline_id(self) -> str:
        return f"address_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [
            ExtractFormattedAddress(),
            FormatAddressStep(self._prop_name, self._prop_schema),
        ]
