...
async def test_create_run_upserts_job_runs(repo, mock_client):
...
async def test_update_job_status_finds_and_saves(repo, mock_client):
    """update_job_status finds run by platform_job_id and saves updated run."""
    mock_client.table.return_value.select.return_value.eq.return_value.limit.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[{
            "id": "run-123",
            "owner_user_id": "871ba2fa-fd5d-4a81-9f0d-0d98b348ccde",
            "job_id": "job1",
...
async def test_get_run_status_returns_status_when_found(repo, mock_client):
    ...
    status = await repo.get_run_status("run-123")
    assert status == "succeeded"


async def test_get_run_status_returns_none_when_not_found(repo, mock_client):
    """get_run_status returns None when run does not exist."""
    mock_client.table.return_value.select.return_value.eq.return_value.limit.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[]
    ))
    status = await repo.get_run_status("run-missing")
    assert status is None


async def test_get_job_retry_count_returns_count_when_found(repo, mock_client):
    ...
    count = await repo.get_job_retry_count("loc_abc")
    assert count == 2


async def test_get_job_retry_count_returns_zero_when_not_found(repo, mock_client):
    """get_job_retry_count returns 0 when run does not exist."""
    mock_client.table.return_value.select.return_value.eq.return_value.limit.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[]
    ))
    count = await repo.get_job_retry_count("loc_missing")
    assert count == 0


async def test_save_job_run_upserts(repo, mock_client):
...
async def test_list_job_runs_by_owner_returns_runs(repo, mock_client):
    """list_job_runs_by_owner returns JobRuns for owner."""
    mock_client.table.return_value.select.return_value.eq.return_value.order.return_value.range.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[
            {
                "id": "run-1",
                "owner_user_id": "871ba2fa-fd5d-4a81-9f0d-0d98b348ccde",
    ...
    runs = await repo.list_job_runs_by_owner("871ba2fa-fd5d-4a81-9f0d-0d98b348ccde")
    assert len(runs) == 1
    assert runs[0].id == "run-1"
    assert runs[0].status == "succeeded"


async def test_list_recent_job_runs_global_order_and_range(repo, mock_client):
    """list_recent_job_runs queries job_runs without owner filter, newest first."""
    mock_client.table.return_value.select.return_value.order.return_value.range.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[
            {
                "id": "run-2",
                "owner_user_id": "871ba2fa-fd5d-4a81-9f0d-0d98b348ccde",
    ...
    runs = await repo.list_recent_job_runs(limit=10, offset=0)
    assert len(runs) == 1
...
async def test_list_recent_job_runs_filters_by_owner_ids(repo, mock_client):
    """list_recent_job_runs uses in_ when owner_user_ids is non-empty."""
    uid = "871ba2fa-fd5d-4a81-9f0d-0d98b348ccde"
    mock_client.table.return_value.select.return_value.in_.return_value.order.return_value.range.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[]
    ))
    await repo.list_recent_job_runs(limit=5, owner_user_ids=[uid])
...
async def test_insert_event_logs_and_does_not_raise(repo):
    """insert_event logs event and does not raise (Phase 4 logs-only observability)."""
    await repo.insert_event("run-123", "pipeline_started", {"job_id": "job1"})
    # No exception; implementation logs only


async def test_save_job_run_raises_on_invalid_status(repo, mock_client):
...
async def test_save_stage_run_emits_string_uuids(mock_resolve, repo, mock_client):
...
@patch("app.repositories.postgres_run_repository.resolve_or_create_mapping")
async def test_save_pipeline_run_emits_string_uuids(mock_resolve, repo, mock_client):
...
@patch("app.repositories.postgres_run_repository.resolve_or_create_mapping")
async def test_save_step_run_emits_string_uuids(mock_resolve, repo, mock_client):
...
@patch("app.repositories.postgres_run_repository.resolve_or_create_mapping")
async def test_save_step_run_includes_processing_log(mock_resolve, repo, mock_client):
    mock_resolve.side_effect = [
        uuid.UUID("d4e5f6a7-b8c9-4012-d345-6789abcdef01"),
        uuid.UUID("e5f6a7b8-c9d0-4123-e456-789abcdef012"),
        uuid.UUID("f6a7b8c9-d0e1-4234-f567-89abcdef0123"),
...
async def test_list_step_runs_for_job_run_orders_and_joins_pipeline(repo, mock_client):
    ...
    step_table.select.return_value.eq.return_value.eq.return_value.order.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[
            {
    ...
    pipe_table.select.return_value.in_.return_value.execute = AsyncMock(return_value=MagicMock(
        data=[{"id": pr_uuid, "pipeline_id": "pipe_a"}]
    ))

    out = await repo.list_step_runs_for_job_run(job_uuid, "871ba2fa-fd5d-4a81-9f0d-0d98b348ccde")
    assert len(out) == 1
...
@patch("app.repositories.postgres_run_repository.resolve_or_create_mapping")
async def test_save_usage_record_emits_string_uuids(mock_resolve, repo, mock_client):
    ...
    await repo.save_usage_record(record)
...
@patch("app.repositories.postgres_run_repository.resolve_or_create_mapping")
async def test_save_usage_record_without_step_run_id_emits_string_uuids(mock_resolve, repo, mock_client):
