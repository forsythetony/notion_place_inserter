"""Custom pipeline for properties we intentionally skip: produce nothing for that field."""
...
class NoOpStep(PipelineStep):
    """Mark property as intentionally skipped. Does not add to payload."""
    ...
    def execute(
        ...
        with log_step(
            ...
            context.mark_property_skipped(self._prop_name)
            return None


class NoOpPipeline(Pipeline):
    """Resolve nothing for this property. Used when we don't want the app to populate the field."""
    ...
    def pipeline_id(self) -> str:
        return f"noop_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [NoOpStep(self._prop_name, self._prop_schema)]
