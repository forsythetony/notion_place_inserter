...
class TargetService:
    ...
    async def get_by_id(
        ...
        return await self._target_repo.get_by_id(target_id, owner_user_id)

    async def list_by_owner(self, owner_user_id: str) -> list[DataTarget]:
        """List all targets for the given owner."""
        return await self._target_repo.list_by_owner(owner_user_id)

    async def save(self, target: DataTarget) -> None:
        """Persist target. Validation is delegated to repository if configured."""
        await self._target_repo.save(target)

    async def delete(self, target_id: str, owner_user_id: str) -> None:
        """Remove target by ID for the given owner."""
        await self._target_repo.delete(target_id, owner_user_id)

    async def get_with_active_schema(
        self, target_id: str, owner_user_id: str
        ...
        """
        target = await self._target_repo.get_by_id(target_id, owner_user_id)
        if target is None:
            return None
        active_schema = None
        if target.active_schema_snapshot_id:
            active_schema = await self._schema_repo.get_by_id(
                target.active_schema_snapshot_id, owner_user_id
            )
        elif target.id:
            active_schema = await self._schema_repo.get_active_for_target(
