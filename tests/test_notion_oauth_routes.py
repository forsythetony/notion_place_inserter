...
def test_notion_oauth_start_200_returns_url(mock_configured, client):
    ...
    mock_svc = MagicMock()
    mock_svc.start_oauth.return_value = "https://api.notion.com/v1/oauth/authorize?client_id=xxx"
    ...
    mock_svc.start_oauth.assert_called_once_with(
...
def test_disconnect_200_returns_status(client):
    ...
    user_id = _setup_auth(client)
    mock_svc = MagicMock()
    mock_svc.disconnect.return_value = ConnectorInstance(
        id=NOTION_CONN_ID,
        owner_user_id=user_id,
    ...
    mock_svc.disconnect.assert_called_once_with(owner_user_id=user_id)
...
def test_select_data_sources_200_creates_targets(client):
    ...
    mock_target_repo.save.assert_called()
