"""Custom pipeline for Latitude: extract from place location (no LLM)."""
...
from app.pipeline_lib.steps.google_places import ExtractLatitude
from app.pipeline_lib.steps.notion_format import format_value_for_notion


class FormatLatitudeStep(PipelineStep):
...
class LatitudePipeline(Pipeline):
    """Resolve latitude from place location. Returns None if not in API response."""
    ...
    def pipeline_id(self) -> str:
        return f"latitude_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [
            ExtractLatitude(),
            FormatLatitudeStep(self._prop_name, self._prop_schema),
        ]
