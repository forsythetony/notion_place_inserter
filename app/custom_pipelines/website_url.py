"""Custom pipeline for Website URL: ExtractWebsiteUri -> format."""
...
from app.pipeline_lib.steps.google_places import ExtractWebsiteUri
from app.pipeline_lib.steps.notion_format import format_value_for_notion


class FormatWebsiteStep(PipelineStep):
    """Format as Notion url."""
...
class WebsiteUrlPipeline(Pipeline):
    """Resolve website from Google Places websiteUri."""
    ...
    def pipeline_id(self) -> str:
        return f"website_{self._prop_name}"

    def steps(self) -> list[PipelineStep]:
        return [
            ExtractWebsiteUri(),
            FormatWebsiteStep(self._prop_name, self._prop_schema),
        ]
